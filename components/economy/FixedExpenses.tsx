  ...
  return (
    <div className="bg-[#111] border border-[#222] rounded-2xl p-5">
      <div className="flex justify-between items-center mb-4">
        <h3 className="font-semibold text-gray-200">Fixed Expenses</h3>
      ...
      <div className="space-y-2 mb-5">
        ...
        {expenses.length === 0 && (
          <p className="text-xs text-gray-500 text-center py-2">No fixed expenses yet.</p>
        )}
      </div>

      <form action={addFixedExpense} className="flex space-x-2">
        <input name="title" placeholder="Gym..." required className="flex-1 bg-[#222] border border-[#333] focus:border-emerald-500 rounded-xl px-4 py-2.5 outline-none text-white text-xs transition-colors" />
        <input name="amount" type="number" step="0.01" placeholder="50" required className="w-24 bg-[#222] border border-[#333] focus:border-emerald-500 rounded-xl px-4 py-2.5 outline-none text-white text-xs transition-colors" />
